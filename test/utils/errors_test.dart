...
// for the new catDebtFree guard (see firestore_service_test.dart for the
...
void main() {
  group('friendlyErrorMessage', () {
    ...
    test('maps "not found" to a pt-BR "no longer exists" message', () {
      expect(
        friendlyErrorMessage(StateError('category not found')),
    ...
    test('maps "source and destination must differ" to a pt-BR message', () {
      expect(
        friendlyErrorMessage(StateError('source and destination must differ')),
    ...
    test('maps an unsupported edit (transfer leg / changing / moving) to a pt-BR "not supported" message', () {
      expect(
        friendlyErrorMessage(StateError('cannot edit a transfer leg directly; recreate the transfer')),
        'Essa alteração não é suportada — remova e lance de novo.',
      );
      expect(
        friendlyErrorMessage(StateError("changing an allocation's caixinha is not supported; delete and recreate")),
        'Essa alteração não é suportada — remova e lance de novo.',
      );
      expect(
        friendlyErrorMessage(StateError('moving an expense between caixinha and account is not supported; delete and recreate')),
        'Essa alteração não é suportada — remova e lance de novo.',
      );
    });
    ...
    test('maps "exceeds"/"overdraw" to a pt-BR "exceeds available balance" message', () {
      expect(
        friendlyErrorMessage(StateError('amount exceeds available balance')),
        'Esse valor ultrapassa o saldo disponível.',
      );
      expect(
        friendlyErrorMessage(StateError('lowering income would overdraw the account')),
        'Esse valor ultrapassa o saldo disponível.',
      );
    });
    ...
    test('maps "cannot be negative"/"must be positive" to a pt-BR "enter a valid value" message', () {
      expect(
        friendlyErrorMessage(StateError('income amount cannot be negative')),
        'Informe um valor válido.',
      );
      expect(
        friendlyErrorMessage(StateError('transfer amount must be positive')),
        'Informe um valor válido.',
      );
    });
    ...
    test('falls back to a generic pt-BR message for an unrecognized error', () {
      expect(
        friendlyErrorMessage(StateError('some new unmapped invariant broke')),
    ...
    test('branch order: "not found" wins over a later-matching substring in the same message', () {
      ...
      expect(
        friendlyErrorMessage(StateError('allocation not found')),
        'Esse lançamento não existe mais.',
      );
    });
  });
