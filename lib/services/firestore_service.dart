...
/// transactions.
///
/// It does NOT limit `fetchAll()` (export/backup, which must stay complete)
/// and, deliberately, it does NOT limit [watchAllIncomes]/
/// [watchAllAllocations]/[watchAllExpenses] either — [summaryProvider]'s
/// Dashboard total is a money-trust number and reads those instead of the
/// windowed streams below precisely so it can never silently undercount an
/// account whose history exceeds this cap (revisits the O(1)-denormalized-
/// read approach tried in an earlier draft of this feature: correct-by-
/// construction is still preferred over a cache for anything the balance
/// display relies on — see the class doc comment above, "MONEY INTEGRITY").
/// Only "histórico mensal" (a lesser display, not a balance) can fall
/// outside this window on a very long-lived account. See docs/BACKEND.md,
/// "read windowing".
...
class FirestoreService {
  ...
  /// The full income ledger, with no `.limit()` — see [_ledgerLimit]'s doc
  /// comment. Used only by [summaryProvider] to compute the Dashboard total;
  /// [watchIncomes] (windowed) still feeds the Receitas list screen.
  ...
  /// The full expense ledger, unwindowed — see [watchAllIncomes].
